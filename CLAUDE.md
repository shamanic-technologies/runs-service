...
- **`total_cost_in_usd_cents` stays GROSS, unchanged.** Every existing gross read returns today's numbers byte-identical. Net is served only where a consumer asks for it: `net_spent_cents` on `GET /internal/org-usage-total` (billing reads it for the spendable balance) and `net{Total,Actual,Provisioned}CostInUsdCents` on GET/POST `/v1/stats/costs` (features reads gross or net per-attribution). Batch/rollup/public reads stay gross-only.
...
- **Discount resolution lives in `src/services/usage-discount.ts`** (billing-discount client — kept OUT of `billing.ts` per its "notifyUsage only" note). It reads `GET /internal/accounts/by-org/{orgId}/usage-discount` from billing-service, with a 60s per-org cache. **Fail loud**: enabled + billing unreachable/5xx/malformed → `UsageDiscountError` → 502 on the cost write; never silently writes gross-as-net. The only zero-discount cases are: resolution disabled, no orgId, or billing 404 (unknown org / no account) → net == gross.
- **Rollout gate `USAGE_DISCOUNT_RESOLUTION_ENABLED` (default OFF).** Until billing-service ships the discount endpoint and ops flips the flag, every org resolves to 0 with NO billing call added to the hot cost-write path — zero regression, zero added latency. Enable only once the billing endpoint is live in the same env (deploy-ordering: producer first). **billing-service dependency:** `GET /internal/accounts/by-org/{orgId}/usage-discount` returning `{ discount_pct }` (fraction in [0,1]) — not yet built; ship it before enabling the flag.
