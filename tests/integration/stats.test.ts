...
describe("Stats endpoints", () => {
  ...
  describe("POST /v1/public/runs/costs", () => {
    it("returns costs for multiple runs", async () => {
      const run1 = await insertTestRun({
        organizationId: TEST_ORG_ID,
        serviceName: "svc",
        taskName: "task",
    ...
    it("includes descendant costs in total", async () => {
      const parent = await insertTestRun({
        organizationId: TEST_ORG_ID,
        serviceName: "svc",
        taskName: "parent",
    ...
    it("excludes cancelled costs", async () => {
      const run = await insertTestRun({
        organizationId: TEST_ORG_ID,
        serviceName: "svc",
        taskName: "task",
      ...
      expect(res.status).toBe(200);
      expect(res.body.costs[0].totalCostInUsdCents).toBe("0.1000000000");
    });

    it("returns zero for non-existent run IDs", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.costs).toHaveLength(1);
      expect(res.body.costs[0].runId).toBe(fakeId);
    ...
    it("does not require auth", async () => {
      const res = await request(app)
        .post("/v1/public/runs/costs")
        .send({ runIds: ["00000000-0000-0000-0000-000000000001"] });

      expect(res.status).toBe(200);
    });

    it("rejects empty runIds", async () => {
      const res = await request(app)
        .post("/v1/public/runs/costs")
        .send({ runIds: [] });
